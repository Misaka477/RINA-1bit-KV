"""Test int4 full attention on real-text NIAH (no top-K, no attractors)."""
import os, sys, torch
...
KW = "KILO42"; WS = 512; CTX = 16384; WARMUP = 10; TOP_K = 99999
...
for depth in [0.25, 0.5, 0.75]:
    ...
    hook = KVRHook(model, window_size=WS, top_k=TOP_K, field_weight=0.0, ret_weight=1.0, device="cuda", num_denoise_steps=0)
    hook.prefill(sq)
    # Disable attractors and chunk scoring → pure token-level, all tokens
    for li in range(model.config.num_hidden_layers):
        hook.retrievals[li].attractors = None
        hook.retrievals[li].labels = None
        hook.retrievals[li].chunk_size = 0
    ...
    print(f"depth={depth:.2f}: {'PASS' if acc else 'FAIL'} {txt[:60]}")
